...
String baseUrl1 = 'https://developmentalphawizz.com/gold_gallery/app/v1/api/';

// 'https://developmentalphawizz.com/gold_gallery/app/v1/api/';
...
Future<SubCategoryModel?> productSubCategoriesCategoriesWise(
    category_id, sub, orderAscDesc, topRated, productType) async {
  var headers = {
    'Cookie':
    'ci_session=me3pm54rt85a7q3gl2an6odso1pr3ci5; ekart_security_cookie=8eaf16bd90e896dc92628c1bcd875530'
  };
  ...
  print(
    "@@ $sub && $orderAscDesc && $topRated && $category_id aand ${request.fields.toString()}, apiiiiiiiiiiiii ${request.url}",
  );
  ...
  if (response.statusCode == 200) {
    final str = await response.stream.bytesToString();
    print("responseeeeeeee ${str}");
    return SubCategoryModel.fromJson(jsonDecode(str));
  } else {
    return null;
  }
}

Future<ProductsModel?> productSubCategoriesCategoriesWiseSort(
    category_id, sub, orderAscDesc) async {
  var headers = {
    'Cookie':
    'ci_session=me3pm54rt85a7q3gl2an6odso1pr3ci5; ekart_security_cookie=8eaf16bd90e896dc92628c1bcd875530'
  };
...
Future<SubCategoriGoldorsilver?> productSubCategoriesCategoriesWiseList(
    id) async {
  var headers = {
    'Cookie':
    'ci_session=me3pm54rt85a7q3gl2an6odso1pr3ci5; ekart_security_cookie=8eaf16bd90e896dc92628c1bcd875530'
  ...
  request.fields.addAll({'id': id, 'product_type_status': "2"});
  print("this is request ${request.fields.toString()}");
...
Future<SubCategoriGoldorsilver?>
productSubCategoriesCategoriesWiseListJewellery(id, subCatId) async {
  var headers = {
    'Cookie':
    'ci_session=me3pm54rt85a7q3gl2an6odso1pr3ci5; ekart_security_cookie=8eaf16bd90e896dc92628c1bcd875530'
  };
  ...
  request.fields.addAll({
    ...
    'child_category': '${subCatId.toString()}'
  });
...
Future<Withdrawmodel?> withDrawApi(
...
Future<PruchaseModel?> purchaseGold(
  ...
  print(
      "this is digi gold and silver buy option $isGold and ${request.toString()} &&&& ${request.fields.toString()}");
...
Future<GetNotificationModel?> getNotification() async {
  var headers = {'Cookie': 'ci_session=5gueac7g05v5t12tbt7nluvihoai58q3'};
  var request = http.Request('POST', Uri.parse(baseUrl1 + 'get_notifications'));
...
Future<VerifyUserDetails> verifyUserToGiftGold(mobileNumber, context) async {
  var headers = {'Cookie': 'ci_session=369m0tojim7hjgo5dt6dlc7skt0cl5kf'};

  var request =
  http.MultipartRequest('POST', Uri.parse(baseUrl + 'get_user_data'));
  request.fields.addAll({'mobile': mobileNumber});
  print("this is request @@ ${request.fields.toString()}");
  ...
  if (response.statusCode == 200) {
    final str = await response.stream.bytesToString();
    if (str == "null") {
      return VerifyUserDetails(message: "Limit Extend");
    } else{
      var result = json.decode(str) ;
      if(!result['error']) {
        return VerifyUserDetails.fromJson(json.decode(str));
      }else{
        return VerifyUserDetails(message: 'User Not Registered !');
      }
    }
  } else {
    return VerifyUserDetails(message: 'User Not Registered !');
...
Future<String> giftGold(
    context, {
      required String userId,
      required String giftTo,
  ...
  var request =
  http.MultipartRequest('POST', Uri.parse('https://developmentalphawizz.com/gold_gallery/app/v1/api/gift_to'));
  request.fields.addAll({
    'user_id': userId,
    'gift_to': giftTo,
    'gift_type': giftType.toLowerCase(),
    'gold_value': giftValue,
    'gold_amount': giftAmount
  });
  print("this is request @@ ${request.fields.toString()}");
  ...
  if (response.statusCode == 200) {
    final str = await response.stream.bytesToString();
    print('${str}___________kdlfjklfjf');
    print("^^ ${str.toString()}");
    if (str == "null") {
      return json.decode(str)['message'];
    } else {
      return json.decode(str)['message'];
    }
  } else {
    return "Unable to Gift Gold";
  }
}

Future<GiftHistoryDetails> getGiftHistoryDetails(userId, context) async {
  var headers = {'Cookie': 'ci_session=369m0tojim7hjgo5dt6dlc7skt0cl5kf'};

  var request =
      http.MultipartRequest('POST', Uri.parse(baseUrl + 'gift_history'));
  request.fields.addAll({'user_id': userId});
  print("this is request @@ ${request.fields.toString()}");
  ...
  if (response.statusCode == 200) {
    ...
    print("^^ ${str.toString()}");
    if (str == "null") {
      return GiftHistoryDetails(message: "Limit Extend");
    } else {
      return GiftHistoryDetails.fromJson(json.decode(str));
    }
  } else {
    return GiftHistoryDetails(message: 'Unable to fetch gift history details');
  }
}
